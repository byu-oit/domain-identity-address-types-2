...
describe('Mock API Unit Tests', function () {
  ...
  it("GET /{address_type} 200", () => {
    return api.request({method: 'GET', path: '/MAL', query: { mock: '' }})
      .then(res => { ut.testMockResponseSuccess(res, 200); });
  });
  it("PUT /{address_type}  200", () => {
    return api.request({ method: 'PUT', path: '/MAL', query: { mock: '' },
      body: {
        "address_type": "MAL",
        "description": "MAL",
        "long_description": "MAL",
  ...
  it("DELETE /{address_type} 204", () => {
    return api.request({ method: 'DELETE', path: '/MAL', query: { mock: '' } })
      .then(res => { ut.testMockResponseSuccess(res, 204); });
  });

  // ----- Country Code Logs -----
  it("GET /{address_type}/logs 200", () => {
    return api.request({ method: 'GET', path: '/MAL/logs', query: { mock: '' } })
      .then(res => { ut.testMockResponseSuccess(res, 200); });
  });
